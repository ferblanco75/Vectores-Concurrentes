...
public class ConcurVector {
	...
	/*******************Desde aqu� Comienzan la Operaciones concurentes ******************/
	...
	public synchronized void abs() {
		...
		this.threadPool.initializeWorkers(operacion,this,aux);
			...
			JOptionPane.showMessageDialog(null, "N�mero inv�lido, no se puede realizar la operaci�n Abs");
	...
	public synchronized double mean() {
		if ( ! (this.dimension()<=0 || this.getThread()<=0 || this.dimension()< this.getThread())){
			...
			JOptionPane.showMessageDialog(null, "N�mero inv�lido, no se puede realizar la operaci�n Mean");
			return 0;
	       } 
	}
	...
	public synchronized double prod(ConcurVector v) {
		...
		return aux.sum();
			...
			JOptionPane.showMessageDialog(null, "N�mero inv�lido, no se puede realizar la operaci�n Prod");
			return 0;
	       } 
	}
	...
	public synchronized double norm() {
		...
		return Math.sqrt(aux.sum());
			...
			JOptionPane.showMessageDialog(null, "N�mero inv�lido, no se puede realizar la operaci�n Norm");
			return 0;
	       } 
	}
	...
	public synchronized void set(double d) {
		if ( ! (this.dimension()<=0 || this.getThread()<=0 || this.dimension()< this.getThread())){
			...
			JOptionPane.showMessageDialog(null, "N�mero inv�lido, no se puede realizar la operaci�n Set (d)");
	...
	public synchronized void assign(ConcurVector vector) {
		if ( ! ((this.dimension()<=0 || this.getThread()<=0 || this.dimension()< this.getThread())
			...
			JOptionPane.showMessageDialog(null, "N�mero inv�lido, no se puede realizar la operaci�n Assign");
	...
	public synchronized void assign(ConcurVector mask, ConcurVector vector){ 
		...
		if ( ! (   (this.dimension()<=0 || this.getThread()<=0 || this.dimension()< this.getThread())
			...
			JOptionPane.showMessageDialog(null, "N�mero inv�lido, no se puede realizar la operaci�n AssignConMask");
	  ...
	  public synchronized void mul(ConcurVector vector) {
		  ...
		  Operacion operacion= new Mul();	
			 ...
			 this.threadPool.initializeWorkers(operacion,this,vector);
				...
				JOptionPane.showMessageDialog(null, "N�mero inv�lido, no se puede realizar la operaci�n Mul");
	 ...
	 * @precondition dimension() == v.dimension(). */
	 public synchronized void add(ConcurVector vector) {
		...
		this.threadPool.initializeWorkers(operacion,this,vector);
		  		...
		  		JOptionPane.showMessageDialog(null, "N�mero inv�lido, no se puede realizar la operaci�n Add");
	...
	/** Obtiene la suma de todos los valores del vector. */
	 ...
	 return ( aux.get(0)); 
					...
					JOptionPane.showMessageDialog(null, "N�mero inv�lido, no se puede realizar la operaci�n Sum");
					return 0;
			       } 
	  ...
	  return ( aux.get(0));
					...
					JOptionPane.showMessageDialog(null, "N�mero inv�lido, no se puede realizar la operaci�n Max");
					return 0;
			       } 
